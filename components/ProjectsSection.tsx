      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
          {BEGINNER_PROJECTS.map((project, idx) => (
            <div key={idx} className="bg-white dark:bg-zinc-950 rounded-xl p-6 border border-zinc-200 dark:border-zinc-800 hover:border-yellow-400 dark:hover:border-yellow-500 transition-colors group shadow-sm">
              <div className="w-10 h-10 bg-zinc-100 dark:bg-zinc-900 rounded-full flex items-center justify-center mb-4 shadow-sm text-zinc-500 dark:text-zinc-400 group-hover:text-yellow-600 dark:group-hover:text-yellow-400 group-hover:bg-zinc-50 dark:group-hover:bg-zinc-800 transition-colors">
              ...
              <div className="mt-auto pt-4 border-t border-zinc-100 dark:border-zinc-800 text-sm font-medium text-zinc-700 dark:text-zinc-300">
                Tool: <span className="text-yellow-600 dark:text-yellow-400">{project.tool}</span>
              </div>
            </div>
          ))}
        </div>
        ...
        {/* Assignment Box */}
        <div className="bg-gradient-to-r from-yellow-400 to-amber-500 rounded-2xl p-8 md:p-12 text-black shadow-xl relative overflow-hidden">
          <div className="absolute top-0 right-0 -mt-10 -mr-10 w-40 h-40 bg-white opacity-20 rounded-full"></div>
          <div className="relative z-10 text-center">
            <Briefcase className="h-12 w-12 mx-auto mb-4 text-black/70" />
            <h3 className="text-2xl font-bold mb-4">{ASSIGNMENT.title}</h3>
            <p className="text-xl md:text-2xl font-bold mb-6">"{ASSIGNMENT.task}"</p>
            <p className="text-black/70 font-medium">OR: {ASSIGNMENT.alternative}</p>
          </div>
        </div>
      </div>
    </section>
